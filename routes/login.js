...
router.post("/", async function(req, res, next) {
  ...
  try {
    ...
    for (i = 0; i < users.length; i++) {
      if (
        users[i].profile.firstName == userinput.firstName &&
        users[i].profile.lastName == userinput.lastName &&
        users[i].profile.userName == userinput.username
      ) {
        if (
          bcrypt.compareSync(userinput.password, users[i].profile.hashPassword)
        ) {
    ...
    res
      .status(401)
      .render("login", { message: "sorry, invalid input, try again" });
  }
});
