...
//-------------------UPDATE--------------------//
async function update(id, nname, ncategory, namount, ncomment, nrecurring){ //make expense
	...
	if (nname !== possibleExp.name && nname !== null){ //then name got change
		//then the name is getting updated
		//repeat this for others
		const newNameChange = await allExpenses.updateOne({_id:expID}, {$set: {name: newname}});
	}
	//if wanna change the category
	if (ncategory !== possibleExp.category && ncategory !== null){ //then name got change
		const newCatChange = await allExpenses.updateOne({_id:expID}, {$set: {category: ncategory}});
	}
	//if wanna change recurring status
	if (nrecurring !== possibleExp.recurring.shouldlRecurr && nrecurring !== null){ //then name got change
		const newRecChange = await allExpenses.updateOne({_id:expID}, {$set: {"recurring.$.amount": nrecurring}});
	}
	//if wanna change amount
	if (namount !== possibleExp.amount && namount !== null){ //then name got change
		const newAmtChange = await allExpenses.updateOne({_id:expID}, {$set: {amount: namount}});
	}
	//if wanna change comment/description
	if (ncomment !== possibleExp.amount && ncomment !== null){ //then name got change
